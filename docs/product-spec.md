...
- **Card** — listed in the picker but **not selectable yet**, labelled "payment gateway pending". Enable it the moment §11.3 is resolved — but the flag alone will not do it. The fee rate is a code constant, not an SA6 field, so enabling card is a deploy that sets the rate followed by a flag flip; the server refuses the flip on its own (§6.2). That asymmetry with the commission rate (§6.1, editable live) is deliberate: a commission change is a pricing decision the owner makes, a gateway fee is a fact about a contract with a provider, and inventing one from an admin screen is how the books start disagreeing with the settlement report. The fee delta (0.8–1.8% vs 3.2–3.65%) must be visible in internal margin reporting — don't let card become invisible overhead.
