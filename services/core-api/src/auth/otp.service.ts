...
export class OtpService {
   ...
   *
   * คืน `devCode` ให้เฉพาะตอนไม่ใช่ production — ยังไม่มีผู้ให้บริการ SMS จริง
   * ถ้าไม่คืนออกมา จะทดสอบ flow สมัครทั้งเส้นไม่ได้เลย
   */
  async request(phone: string): Promise<{ expiresAt: Date; devCode?: string }> {
    ...
    // (เบอร์ซ้ำไม่ได้อยู่แล้วตาม unique index จึงไม่ได้เปิดเผยอะไรที่หน้าสมัครไม่บอกอยู่ดี)
    if (existing) {
   ...
   * ถ้าเช็คแค่ลายเซ็น คนที่ยืนยันเบอร์ตัวเองแล้วจะเอาตั๋วไปสมัครด้วยเบอร์คนอื่นได้
   */
