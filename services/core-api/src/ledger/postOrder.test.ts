...
describe('postOrderDelivered', () => {
  ...
  it('ใช้อัตราคอมของออเดอร์ใบนั้น ไม่ใช่ค่าตั้งต้นของระบบ', () => {
    ...
    expect(restaurantPayableAt(1500)).toBe(17000); // 15% → ร้านได้ ฿170
    expect(restaurantPayableAt(1200)).toBe(17600); // 12% → ร้านได้ ฿176
    expect(restaurantPayableAt(0)).toBe(20000); // ไม่เก็บคอม → ร้านได้เต็ม
  });
