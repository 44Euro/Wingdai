...
export class RefundsService {
  ...
  /** ลูกค้าแจ้งปัญหา ระบบตรวจอัตโนมัติแล้วเก็บข้อเสนอไว้รอแอดมิน (product-spec §6.4) */
    ...
    await this.db.transaction(async (tx) => {
      ...
      await tx.insert(ledgerEntries).values(
        postRefund({ amountSatang: amount, fault }).map((l) => ({
          ...
          orderId: order.id,
          /** คืนเงินที่เป็นความผิดของร้านเดบิต `restaurant_payable` ต้องติดร้านไปด้วย */
      ...
      await writeAudit(tx, {
        ...
        subjectType: 'refund_case',
        subjectId: caseId,
        before: { status: row.status, suggestedAmountSatang: row.suggestedAmountSatang },
        after: { status: 'approved', approvedAmountSatang: amount, fault },
      });
    });
