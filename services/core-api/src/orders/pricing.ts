...
/**
 * ค่าธรรมเนียมเกตเวย์ต่อหนึ่งหมื่น (basis point) — `null` คือ **ยังไม่รู้อัตรา** ไม่ใช่ศูนย์
 *
 * สองอย่างนี้ต่างกันและเคยใส่เลขตัวเดียวกันจนแยกไม่ออก เงินสดเป็น 0 เพราะไม่มีเกตเวย์ให้เสีย
 * ตั้งแต่ต้นและจะเป็น 0 ตลอดไป ส่วนพร้อมเพย์กับบัตรยังไม่มีตัวเลขเพราะ §11.3 ยังไม่ตอบว่า
 * ใช้เกตเวย์เจ้าไหน — คนละเหตุผล คนละอนาคต
 *
 * ตัวอย่างใน §6.2 (ออเดอร์ ฿170 เสียค่าธรรมเนียม ฿1.36 = 80 bp) จึงเป็นรูปของรายการ
 * **หลัง** เชื่อมเกตเวย์แล้ว ไม่ใช่ตัวเลขที่ระบบวันนี้ผลิตออกมา
 */
export const PAYMENT_FEE_BP: Record<'promptpay' | 'cash' | 'card', number | null> = {
  promptpay: null,
  cash: 0,
  card: null,
};
...
/** อัตราที่ยังไม่รู้คิดเป็น 0 — ไม่มีใครเก็บจริง การเดาตัวเลขคือลงบัญชีค่าใช้จ่ายที่ไม่เคยเกิด */
