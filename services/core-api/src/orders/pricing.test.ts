...
describe('การคิดยอดออร์เดอร์', () => {
  it('ค่าอาหารคิดจากราคาต่อหน่วยคูณจำนวน', () => {
    const p = priceOrder([item({ quantity: 2 }), item({ unitPriceSatang: 2500 })]);
    expect(p.foodTotalSatang).toBe(12500);
  });

  it('ค่าส่งกับค่าบริการเป็นบรรทัดแยก ไม่ยุบเข้าค่าอาหาร (claude.md §3 ข้อ 2)', () => {
    const p = priceOrder([item()]);
    expect(p.foodTotalSatang).toBe(5000);
    expect(p.deliveryFeeSatang).toBe(DELIVERY_FEE_SATANG);
    expect(p.serviceFeeSatang).toBe(SERVICE_FEE_SATANG);
    expect(p.grandTotalSatang).toBe(5000 + DELIVERY_FEE_SATANG + SERVICE_FEE_SATANG);
  });
  ...
  it('คอมมิชชันเป็น 15% ของค่าอาหารเท่านั้น ไม่คิดจากค่าส่ง/ค่าบริการ', () => {
    const p = priceOrder([item({ unitPriceSatang: 15000 })]);
  ...
  it('ยอดทุกช่องเป็นจำนวนเต็มสตางค์เสมอ', () => {
    ...
    const p = priceOrder([item({ unitPriceSatang: 3333, quantity: 3 })]);
...
describe('ค่าธรรมเนียมเกตเวย์', () => {
   ...
   * เทสต์นี้จะพังตอนใส่อัตราจริง — นั่นคือเจตนา ให้กลับมาอ่านเหตุผลก่อนแก้ตัวเลข
   */
