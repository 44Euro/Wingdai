...
export class OrdersService {
  ...
  async tip(orderId: string, customerId: string, amountSatang: number): Promise<PublicOrder> {
    ...
    await this.db.transaction(async (tx) => {
       ...
       * วันที่ §11.3 ได้คำตอบ อัตราพร้อมเพย์ใน `PAYMENT_FEE_BP` ต้องถูกตั้งพร้อมกับที่เปิดประตู
       * ไม่งั้นทิปจะลงบัญชีเต็มยอดโดยไม่มีบรรทัดค่าธรรมเนียม ซึ่ง §6.2 ห้ามไว้
       */
