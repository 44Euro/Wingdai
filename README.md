...
`build:web` chains `scripts/prepare-web.mjs`, which does two things a plain `expo export` does not.

Expo emits the bundled fonts under `assets/node_modules/...`, and Vercel drops every path
containing `node_modules` at upload time — a `!` line in `.vercelignore` cannot bring back files
inside a directory that was excluded wholesale. So the script moves them to `assets/vendor/` and
rewrites the paths the bundle refers to. It then fails loudly if anything is still under
`node_modules`, because the failure mode is silent: fonts 404 and Latin text falls back to a serif.

It also writes `dist/vercel.json` with the SPA rewrite. Uploading a prebuilt folder means
`apps/mobile/vercel.json` is never read, so without this every path except `/` returns 404 —
which kills `restaurant/:id` and `order/:id` from `src/app/linking.ts`, the links the merchant
QR screen prints, and plain page refresh.
...
```bash
cd apps/mobile       && npx jest            # 886 tests — screens, stores, pricing, i18n, contrast
...
- **Not in an app store.** The web build is deployed and public, but store submission needs a PDPA
  data controller identity — a legal fact rather than an engineering one. The consent screen is
  built and says so on its own face: it is a sample document, not a binding one.
- **Shared demo state.** Everyone hits the same database, so anything one visitor changes the next
  visitor sees. A GitHub Actions cron wipes and reseeds it nightly at 02:00 ICT, so the damage has
  a 24-hour ceiling — but within a day, visitors see each other's edits.

Food and storefront photos come from Wikimedia Commons under CC BY / CC BY-SA / CC0. Each file,
