...
import { CountdownRing } from '../../../ui/motion';
...
export function RiderHomeScreen({ navigation }: Props) {
  ...
  return (
    ...
    >
      <ScrollView
        contentContainerStyle={{ paddingBottom: RIDER_TAB_CLEARANCE, gap: p.space.lg }}
        ...
        <View style={{ paddingHorizontal: p.space.screen }}>
          <Card>
            <View style={{ gap: p.space.sm }}>
              ...
              {/* รายได้ ถอนเงิน จุดตั้งทำงาน ย้ายไปอยู่แท็บกับจอโปรไฟล์แล้ว */}
            </View>
          </Card>
        </View>
    ...
    <Card tone="teal" testID="rider-offer">
      <View style={{ gap: p.space.md }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {/* วงแหวนนับถอยหลังตามดีไซน์ ตัวเลขอยู่กลางวง ไม่ใช่แค่ตัวเลขเปล่า */}
          <View style={{ width: 64, height: 64, alignItems: 'center', justifyContent: 'center' }}>
            <View style={{ position: 'absolute' }}>
              <CountdownRing
                seconds={RESPOND_WINDOW_SECONDS}
                size={64}
                stroke={5}
                color={tokens.brandAccent}
                trackColor={tokens.textOnTealMuted}
              />
            </View>
            <Text testID="offer-countdown" variant="h3" color="onTeal" bold>
              {secondsLeft}
            </Text>
          </View>
        </View>
