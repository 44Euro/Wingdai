...
import { Card, PhotoBlock, Checkbox } from '../../../ui/Surface';
  ...
  return (
    <SafeAreaView testID="screen-cart" edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: tokens.bgSurface }}>
      ...
      {restaurant ? (
        <View
          style={{
            ...
            paddingHorizontal: p.space.screen, paddingBottom: 6,
          }}
        >
          ...
          {/* C31 ล้างทั้งใบในที่เดียว ไม่ต้องกดลบทีละรายการ */}
          <Pressable
          ...
          >
            <Text variant="caption" color="brand" bold>{t('customer.cart.clear')}</Text>
          </Pressable>
        </View>
      ) : null}
      ...
      >
        ...
        {/* สรุปค่าใช้จ่าย ค่าส่ง/ค่าบริการแยกบรรทัดเสมอ ห้ามบวกทับราคาอาหาร (product-spec §3) */}
        ...
        {/* วางไว้หน้าประตู (สเปคคลื่น 2 §7) ลูกค้าเป็นคนขอตรงนี้ ไม่ใช่ไรเดอร์ติ๊กตอนส่ง */}
        <Card>
          <Checkbox
            testID="leave-at-door"
            checked={cart.leaveAtDoor}
            onChange={cart.setLeaveAtDoor}
            label={t('customer.cart.leaveAtDoor')}
          />
          <Text variant="caption" color="muted" style={{ marginTop: p.space.xs }}>
            {t('customer.cart.leaveAtDoorHint')}
          </Text>
        </Card>
      </ScrollView>
