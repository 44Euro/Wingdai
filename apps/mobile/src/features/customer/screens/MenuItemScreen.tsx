...
export function MenuItemScreen({ navigation, route }: Props) {
  ...
  function add() {
    ...
    addLine(restaurantId, { menuItem: item, selectedChoices: choices, quantity, note });
    navigation.goBack();
  }
    ...
    <SafeAreaView testID="screen-menu-item" edges={['bottom']} style={{ flex: 1, backgroundColor: tokens.bgSurface }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 130 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ paddingHorizontal: p.space.screen, paddingTop: p.space.lg, gap: p.space.lg }}>
          ...
          {/* ฝากข้อความถึงร้านสำหรับจานนี้ วางเหนือปุ่มเลือกจำนวน */}
          <Field label={t('customer.item.noteLabel')} hint={t('customer.item.noteHint')}>
            <Input
              testID="input-item-note"
              accessibilityLabel={t('customer.item.noteLabel')}
              value={note}
