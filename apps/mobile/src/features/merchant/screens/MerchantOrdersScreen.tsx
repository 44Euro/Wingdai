 ...
 *
 * claude.md §10 ห้ามใช้ blur ที่จอนี้เด็ดขาด (`expo-blur` ถูก emulate บน Android
 * และกินเฟรม) จอนี้กับจอรับงานไรเดอร์เป็นสองจอที่พลาดแล้วเจ็บที่สุด จึงใช้พื้นทึบล้วน
 */
            ...
            <Card>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: p.space.md }}>
                ...
                {/*
                  ...
                  ใบที่รับไว้แล้วยังต้องทำต่อ — คิวด้านล่างจึงไม่หายไปตอนกดปิด
                */}
                <Toggle
        ...
        <View style={{ gap: p.space.sm }}>
          ...
          <View style={{ gap: 2 }}>
            {order.items.map((i, idx) => (
              <Text key={`${i.name}-${idx}`} variant="small" color="muted">
                {i.quantity}× {i.name}
              </Text>
            ))}
          </View>
          ...
          {/*
            ...
            ค่าส่งกับค่าบริการไม่ใช่เงินของร้าน โชว์รวมไปมีแต่ทำให้ร้านคาดหวังผิด (claude.md §6.1)
          */}
          <View
            style={{
              flexDirection: 'row',
