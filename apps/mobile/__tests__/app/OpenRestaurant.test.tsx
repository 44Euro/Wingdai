...
describe('OpenRestaurantScreen — เปิดร้านของคุณ (§4.3)', () => {
  ...
  /**
   * §1 — ร้านนอกโซนทำลายสมมติฐานความหนาแน่นที่ GP 15% ยืนอยู่
   * เซิร์ฟเวอร์เป็นคนตัดสิน จอแค่แสดงเหตุผลที่ได้กลับมา
   */
  it('พิกัดนอกโซนถูกปฏิเสธพร้อมบอกเหตุผล', async () => {
    await act(async () => {
      await useAuthStore.getState().login('somchai', '1234');
    });
    ...
    expect(findAll(result.root, 'restaurant-submitted').length).toBe(0);
  });
