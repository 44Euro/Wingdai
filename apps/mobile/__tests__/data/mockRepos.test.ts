...
describe('MockRepo — auth', () => {
  ...
  // อีเมลเป็นแค่ช่องทางรีเซ็ตรหัส ไม่ใช่ identifier (product-spec §4.2 ปรับ 2026-07-29)
...
describe('catalog.getMenu + orders guard', () => {
  ...
  it('เจ้าของร้านสั่งร้านตัวเอง → ถูกบล็อกที่ชั้น repo (throw)', async () => {
    const repos = createMockRepos();
    // มาลีเป็นเจ้าของครัวมาลี ล็อกอินเป็นมาลีแล้วสั่งครัวมาลีต้องไม่ได้ (product-spec §4.3)
